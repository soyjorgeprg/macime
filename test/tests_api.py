...
async def test_mejores_api_correcta(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "mejorES/GLP/37.17881562595606/-3.6087917405863004"
...
async def test_mejores_api_fallo_coord(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "mejorES/GNC/37.17881562595606/prueba"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_mejores_api_fallo_tipo(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "mejorES/diesel/37.17881562595606/-3.6087917405863004"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_cercanas_api_correcta(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "cercanas/GLP/37.17881562595606/-3.6087917405863004"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_cercanas_api_fallo_coord(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "cercanas/GNC/37.17881562595606/prueba"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_cercanas_api_fallo_tipo(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "cercanas/diesel/37.17881562595606/-3.6087917405863004"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_x_cercanas_api_correcta(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "cercanas/GLP/37.17881562595606/-3.6087917405863004/5"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_x_cercanas_api_fallo_coord(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "cercanas/GNC/37.17881562595606/prueba/5"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_x_cercanas_api_fallo_tipo(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "cercanas/diesel/37.17881562595606/-3.6087917405863004/5"
    response = await test_client.get(URI)
    text = await response.text()
...
async def test_x_cercanas_api_fallo_num(test_client: TestClient):
    conf = prep()
    URI = conf['config.api']['api-path'].lstrip("\'").rstrip("\'") + "cercanas/diesel/37.17881562595606/-3.6087917405863004/x"
    response = await test_client.get(URI)
    text = await response.text()
